 ...
 * No diet plan — that feature is handled separately.
 */
...
const MAX_OCR_CHARS_FOR_AI = 4000;
const MAX_AI_OUTPUT_TOKENS = 1100;
...
CRITICAL RULES:
- Return ONLY valid JSON. Never return markdown or prose.
- Never omit any field from the schema.
- Keep all descriptions short (1-2 sentences max).
- Keep recommendations practical and actionable.

NUMERIC FIELDS — these MUST contain only a number as a string:
- metabolismInsights.bmr → e.g. "1720"  (never "Estimated BMR is 1720 kcal")
- metabolismInsights.metabolicAge → e.g. "24"  (never "Metabolic age is 24")
- visceralFatAnalysis.level → e.g. "7"  (never "Normal" or "Level 7")

Return this exact JSON schema:
{
  "overallSummary": "",
  "fitnessLevel": "",
  "bodyFatAnalysis": { "status": "", "description": "", "recommendation": "" },
  "muscleMassAnalysis": { "status": "", "description": "", "recommendation": "" },
  "metabolismInsights": { "bmr": "", "metabolicAge": "", "description": "" },
  "visceralFatAnalysis": { "level": "", "risk": "", "recommendation": "" },
  "strengths": [""],
  "weaknesses": [""],
  "healthRisks": [""],
  "recommendations": [""],
...
export async function analyzeWithGemini(
  ...
  const prompt = `Analyze this InBody report:\n\n${JSON.stringify(payload)}`;
  ...
  try {
    ...
    logger.warn({ err: err.message }, "Primary model failed");
  }
  ...
  try {
    ...
    logger.error({ err: err.message }, "Fallback model failed");
    return withAiDebug(getDemoAnalysis(metrics), {
      source: "fallback",
      model: FALLBACK_GROQ_MODEL,
      rawResponse: "",
...
function extractImportantMetricsFromOCR(
  ...
  function pick(patterns: RegExp[]): string | undefined {
    for (const p of patterns) {
      ...
      if (m?.[1]) {
        // Collapse spaces within number: "1 7 2 0" → "1720", "40. 0" → "40.0"
        const cleaned = m[1]
...
async function requestGroqAnalysis(
  ...
  // Sanitize: ensure numeric fields contain only numbers
  parsed.metabolismInsights = parsed.metabolismInsights ?? {};
  parsed.visceralFatAnalysis = parsed.visceralFatAnalysis ?? {};
  parsed.bodyFatAnalysis = parsed.bodyFatAnalysis ?? {};
  parsed.muscleMassAnalysis = parsed.muscleMassAnalysis ?? {};
...
function getDemoAnalysis(
  ...
  const weight = parseFloat(metrics.weight ?? "64");
  const bodyFat = parseFloat(metrics.bodyFat ?? "15");
  const bmi = parseFloat(metrics.bmi ?? "22");
  const smm = parseFloat(metrics.skeletalMuscleMass ?? "30");
  const bmr = parseInt(metrics.bmr ?? "1700", 10);
  const visceralFat = parseInt(metrics.visceralFat ?? "7", 10);
  const metabolicAge = metrics.metabolicAge ?? "24";
  ...
  return {
    ...
    fitnessLevel:
      bodyFat < 15 ? "Advanced" : bodyFat < 22 ? "Intermediate" : "Beginner",

    bodyFatAnalysis: {
      status: bodyFat < 15 ? "Low" : bodyFat < 22 ? "Normal" : "High",
      description:
        bodyFat < 15
    ...
    muscleMassAnalysis: {
      status: smm > 34 ? "High" : smm > 28 ? "Average" : "Low",
      description:
        smm > 28 ? "Muscle mass is decent." : "Muscle mass can improve.",
      recommendation: "Train with progressive overload 4x weekly.",
    },

    metabolismInsights: {
      bmr: String(bmr),
      metabolicAge: String(metabolicAge),
      description: "Metabolism is stable and healthy.",
    },

    visceralFatAnalysis: {
      level: String(visceralFat),
      risk: visceralFat <= 9 ? "Low" : visceralFat <= 12 ? "Moderate" : "High",
      recommendation: "Maintain cardio and active lifestyle.",
    },
    ...
    weaknesses: [
      smm < 30 ? "Low muscle mass" : "Could improve muscle definition",
    ],

    healthRisks: [
      bmi > 25 ? "Elevated BMI" : "No major risks detected",
    ],
    ...
    workoutPlan: {
      goal: bodyFat > 20 ? "Fat Loss" : "Lean Muscle Gain",
      planType: bodyFat > 20 ? "Recomposition" : "Hypertrophy",
      weeklySchedule: [
        {
          day: "Monday",
          focus: "Push Workout",
      ...
      cardioRecommendation: "2-3 cardio sessions weekly.",
    },
